...
import {
  ComposedChart, Line, Bar, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend, 
  ResponsiveContainer, ReferenceLine, ReferenceArea, Cell
} from 'recharts';
import { 
  Activity, TrendingUp, TrendingDown, AlertTriangle, DollarSign, Trophy, Skull,
  PlayCircle, Info, Settings, Calculator, History, BarChart2, FileText, Layers, 
  ...
  useEffect(() => {
    const fetchStructure = async () => {
      ...
      try {
        ...
        setError("Failed to load Option Strip. Check Backend."); 
      } 
...
// --- PAGE 2: TERMSHEET (Updated with Description) ---
             ...
             {!simulationResult ? (
                ...
                <ResponsiveContainer width="100%" height={250}>
                    <ComposedChart data={simulationResult.chart_data}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#eee" /><XAxis dataKey="date" hide /><YAxis domain={['auto', 'auto']} tick={{fontSize:10}} width={40} /><ReferenceArea yAxisId="0" y1={zoneY1} y2={zoneY2} fill="#10b981" fillOpacity={0.08} /><ReferenceLine y={data.strike_price} stroke="#007fa3" strokeDasharray="3 3" /><ReferenceLine y={data.ko_price} stroke="#971B2F" strokeDasharray="3 3" /><Bar dataKey="chart_pnl" fill="#8884d8" barSize={2}>{simulationResult.chart_data.map((e,i) => <Cell key={i} fill={e.pnl>=0 ? '#008675' : '#971B2F'} opacity={0.8}/>)}</Bar><Line type="monotone" dataKey="spot" stroke="#001489" dot={false} strokeWidth={1.5} />
                    </ComposedChart>
                </ResponsiveContainer>
             )}
          </div>
...
// --- PAGE 3: SIMULATOR ---
const SimulatorView = ({ formData, setFormData, result, setResult }) => {
  ...
  const handleProductChange = (e) => {
    ...
    let updates = { product_type: newType };
    if (newType === 'Decumulator') { updates.strike_price = 1.1200; updates.ko_price = 1.0500; } 
    else { updates.strike_price = 1.0500; updates.ko_price = 1.1200; }
  ...
  if (result) {
    ...
    if (activeDays.length > 0) {
        ...
        if (fixingDays.length >= formData.gearing_limit) gearingEndDate = fixingDays[formData.gearing_limit - 1].date;
        else if (fixingDays.length > 0) gearingEndDate = fixingDays[fixingDays.length - 1].date; 
    }
  }
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-content">
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group"><label>Type</label><select className="form-control" name="product_type" value={formData.product_type} onChange={handleProductChange}><option value="Accumulator">Accumulator (Buy)</option><option value="Decumulator">Decumulator (Sell)</option></select></div>
            <div className="form-group"><label>Ticker</label><input className="form-control" type="text" value={formData.ticker} onChange={(e)=>setFormData({...formData, ticker:e.target.value})} /></div>
            <div className="form-section-title">Schedule</div>
            <div className="input-row"><div className="form-group"><label>Start</label><input className="form-control" type="date" value={formData.start_date} onChange={(e)=>setFormData({...formData, start_date:e.target.value})} /></div><div className="form-group"><label>End</label><input className="form-control" type="date" value={formData.end_date} onChange={(e)=>setFormData({...formData, end_date:e.target.value})} /></div></div>
            <div className="form-group"><label><Calendar size={14}/> Frequency</label><select className="form-control" value={formData.frequency} onChange={(e)=>setFormData({...formData, frequency:e.target.value})}><option value="Daily">Daily</option><option value="Weekly">Weekly (Fri)</option><option value="Monthly">Monthly (End)</option></select></div>
            <div className="form-section-title">Economics</div>
            <div className="form-group"><div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}><label>Strike</label> <TargetButton target="strike_price"/></div><input className="form-control" type="number" step="0.0001" value={formData.strike_price} onChange={(e)=>setFormData({...formData, strike_price:parseFloat(e.target.value)})} disabled={solveTarget === 'strike_price'} /></div>
            <div className="form-group"><div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}><label>Barrier</label> <TargetButton target="ko_price"/></div><input className="form-control" type="number" step="0.0001" value={formData.ko_price} onChange={(e)=>setFormData({...formData, ko_price:parseFloat(e.target.value)})} disabled={solveTarget === 'ko_price'} /></div>
            <div className="form-group"><label>Notional</label><input className="form-control" type="number" value={formData.notional} onChange={(e)=>setFormData({...formData, notional:parseFloat(e.target.value)})} /></div>
            <div className="gearing-panel" style={{marginTop: '10px'}}>
              <div className="gearing-header"><Gauge size={16}/> Gearing Setup</div>
              <div className="input-row">
                  <div className="form-group"><div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}><label>Leverage</label> <TargetButton target="leverage"/></div><input className="form-control" type="number" step="0.1" value={formData.leverage} onChange={(e)=>setFormData({...formData, leverage:parseFloat(e.target.value)})} disabled={solveTarget === 'leverage'} /></div>
                  <div className="form-group"><label># Fixings</label><input className="form-control" type="number" value={formData.gearing_limit} onChange={(e)=>setFormData({...formData, gearing_limit:parseInt(e.target.value)})} /></div>
              </div>
            </div>
...
const ValuationView = ({ sharedData }) => {
  ...
  useEffect(() => {
    if(sharedData.start_date && sharedData.end_date) {
      ...
      setMarketInputs(prev => ({...prev, days_to_expiry: diffDays}));
    }
...
function App() {
  ...
  const [globalData, setGlobalData] = useState({
    ticker: 'EURUSD=X', start_date: '2024-01-01', end_date: '2024-12-31',
    product_type: 'Accumulator', frequency: 'Weekly', strike_price: 1.0500, ko_price: 1.1200,
    notional: 1000, leverage: 2.0, gearing_limit: 252
  });
  ...
  return (
    <div className="app-layout">
      ...
      {view === 'simulator' && <SimulatorView formData={globalData} setFormData={setGlobalData} result={simulationResult} setResult={setSimulationResult} />}
